...
impl Egl实现 {
    pub fn new(要求: Gl要求, 原生窗口: &NativeWindow) -> Result<Self, String> {
        let 库 = 加载库()?;
        ...
        let (显示, 配置, 语境, 表面, 类型) = unsafe {
            ...
            let 表面指针 = 原生窗口.ptr().as_ptr() as *const _;
    ...
    pub fn 重新创建表面(&mut self, 原生窗口: &NativeWindow) -> Result<(), String> {
        unsafe {
            let 表面指针 = 原生窗口.ptr().as_ptr() as *const _;
            self.封装.表面 =
                创建窗口表面(self.封装.库, self.封装.显示, self.封装.配置, 表面指针)?;
        }
